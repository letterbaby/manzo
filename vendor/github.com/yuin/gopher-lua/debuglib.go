...
func debugGetInfo(L *LState) int {
	...
	tbl.RawSetString("func_", LString(fmt.Sprintf("%p", fn)))
	L.Push(tbl)
	return 1
}
...
func newDebugHook(callback *LFunction, event LString) *debugHook {
	return &debugHook{
		callback: callback,
		event:    event,
	}
...
//cf是base，当前的在L中
func (self *debugHook) call(L *LState, cf *callFrame) {
	if self.callback != cf.Fn {
		L.reg.Push(self.callback)
		L.reg.Push(self.event)
		L.callR(1, 0, -1)
	}
}
